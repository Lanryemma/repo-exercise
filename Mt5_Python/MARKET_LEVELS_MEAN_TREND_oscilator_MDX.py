...
#     '1H': {'fast': 34, 'slow': 89}
# }
def calculate_trend(DF, fast_period=1, slow_period=1):
    df = DF.copy()
    df['fast_ema'] = df['close'].ewm(span=fast_period).mean()
    df['slow_ema'] = df['close'].ewm(span=slow_period).mean()
    df['signal1'] = 0  # Initialize signal column
    
    # Generate signals (1 = uptrend, -1 = downtrend)
    df.loc[df['fast_ema'] > df['slow_ema'], 'signal1'] = 1
    df.loc[df['fast_ema'] < df['slow_ema'], 'signal1'] = -1
...
def calculate_indicators(DF, params):
    df = DF.copy()
    """Calculate all technical indicators with dynamic parameters"""
    # Moving Averages
    df['rma'] = df['close'].ewm(span=params['rma_len'], adjust=False).mean()
    df['sma'] = df['close'].rolling(params['sma_len']).mean()
    ...
    avg_gain = gain.ewm(alpha=1/params['rsi_len'], adjust=False).mean()
    avg_loss = loss.ewm(alpha=1/params['rsi_len'], adjust=False).mean()
    ...
    df['atr'] = tr.rolling(params['atr_len']).mean()
...
def generate_signals(df):
    """Generate consensus trend signals (1 = uptrend, -1 = downtrend)"""
    conditions = pd.DataFrame()
    
    # MA-based signals using ATR bands
    for ma in ['rma', 'sma', 'ema']:
        ma_upper = df[ma] + df['atr']
        ma_lower = df[ma] - df['atr']
        conditions[f'{ma}_long'] = df['close'] > ma_upper
        conditions[f'{ma}_short'] = df['close'] < ma_lower
    
    # RSI signal
    conditions['rsi_long'] = df['rsi'] > 50
    conditions['rsi_short'] = df['rsi'] < 50
    
    # Calculate consensus score
    long_votes = conditions.filter(like='_long').sum(axis=1)
    short_votes = conditions.filter(like='_short').sum(axis=1)
    df['signal2'] = 0
    df.loc[long_votes > short_votes, 'signal2'] = 1
    df.loc[short_votes > long_votes, 'signal2'] = -1
...
if __name__ == "__main__":
    ...
    result1 = calculate_trend(data, fast_period=13, slow_period=34)
    ...
    PARAM_SETTINGS = {
        '5M': {'rma_len': 8, 'sma_len': 12, 'ema_len': 20, 
                'rsi_len': 14, 'atr_len': 14},
        '15M': {'rma_len': 12, 'sma_len': 18, 'ema_len': 30, 
                'rsi_len': 20, 'atr_len': 18},
        '30M': {'rma_len': 18, 'sma_len': 24, 'ema_len': 45, 
                'rsi_len': 22, 'atr_len': 20},
        '1H': {'rma_len': 24, 'sma_len': 36, 'ema_len': 60, 
                'rsi_len': 24, 'atr_len': 24}
    }
    params = PARAM_SETTINGS['15M']
    
    result2 = calculate_indicators(data, params) #for M1-M5 (short_ema=10, long_ema=38) | for M15-H1 (short_ema=12, long_ema=48) |
