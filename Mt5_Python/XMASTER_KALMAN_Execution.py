...
params = pd.read_csv("C:\\Users\\user\\Downloads\\params_2.csv")
symbols = params.Symbol.to_list()
print(params)


def get_hist_data_numeric_index(symbol, timeframe, start_pos=0, num_candles=300):
    """
    Parameters
    ----------
...
def trade_signal(data,l_s):
    signal = ""
    sig1_index = data.columns.to_list().index("signal1")
    sig2_index = data.columns.to_list().index("signal2")
    ...
    if l_s == "":
        ...
        if ((data.iloc[-2,sig1_index]==1) &  (data.iloc[-2,sig2_index]==-1)): #and session_active: #-2 refers to the last completed candle because in all likelihood the last candle in ohlc dataframe would be an unfinished candle.
            signal = "Buy"
        elif ((data.iloc[-2,sig1_index]==-1) &  (data.iloc[-2,sig2_index]==1)): #and session_active:
            signal = "Sell"
...
def main(symbol):
    ...
    try:
        ...
        result1 = calculate_signals(data1, short_len=50, long_len=150)
        result2 = calculate_xmaster_signals(data1, short_ema=12, long_ema=48)
        data1["signal1"] = result1["signal1"]       
        data1["signal2"] = result2["signal2"] 
        data1['volatility'] = calculate_volatility(data1, 34, 2.4)
        data1['sar'] = parabolic_sar(data1,step=0.01, max_step=0.2)
        ...
        if signal == "Buy":
            ...
            sl_pips = max(1.0 * atr.iloc[-2], min_stop_pips)  # Ensure minimum stop
